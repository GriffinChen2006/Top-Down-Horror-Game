...
config.hitboxes						= {script.Parent.Parent.Hitbox1, script.Parent.Parent.Hitbox2}		-- As default, HumanoidRootPart is used (NOT recommended to do GetChildren() on AI).
config.damageDelay 					= 2	-- In seconds, how long until the AI can damage again (or move if setting below is enabled)
config.disable_ai_while_damaging 	= true -- Slowdown
config.damageDone 					= 10
config.chaseSpeed					= 24
config.crouchFactor 				= 0
config.walkFactor 					= 5
config.sprintFactor					= 15
config.wanderSightFactor 			= 1
config.wanderPenaltyFactor 			= 1
config.wanderFOVFactor 				= 1
config.enragedSightFactor 			= 1.5
config.enragedPenaltyFactor			= 3
config.enragedFOVFactor 			= 2
config.sightFactor 					= 1
...
config.detectionRange				= 15
config.detectionFOV					= 90		-- In degrees, the detection FOV of the AI. LIMIT: 180 for full 360 degrees.
