...
local npcInChase = script.Parent.InChase
local npcPersistence = npcInChase.Persistence
local npcHumanoid = script.Parent:FindFirstChild("Humanoid")
local bite = script.Parent.Head.Bite
...
if config.PreventAIFromSitting then
	config.enemy_char:WaitForChild("Humanoid"):SetStateEnabled(Enum.HumanoidStateType.Seated, false)
	config.enemy_char:WaitForChild("Humanoid"):SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)
	config.enemy_char:WaitForChild("Humanoid"):SetStateEnabled(Enum.HumanoidStateType.Ragdoll, false)
	config.enemy_char:WaitForChild("Humanoid"):SetStateEnabled(Enum.HumanoidStateType.Climbing, false)
end
	...
	if config.disable_ai_while_damaging then
		...
		if not damaged_recently.Value then
			...
			spawn(function()
				...
				script.Parent:WaitForChild("Humanoid").WalkSpeed = config.chaseSpeed
			end)
		end
...
local function isInView(plr_char: Model) -- Determines if a model is in the view of the AI.
	...
	local plr_hrt = plr_char:WaitForChild("HumanoidRootPart")
	...
	detectionFactor *= config.penaltyFactor
	detectionFactor *= class_factor

	FOV = config.detectionFOV * config.FOVFactor

	local result = std.math.LineOfSight(config.enemy_char, plr_char, {range = config.detectionRange * config.sightFactor + detectionFactor, SeeThroughTransparentParts = config.seeThroughTransparent, filterTable = {config.enemy_char, chasePart}})
...
local function touchHandler(hit) -- HELPER
	...
	if hit.Parent:IsA("Accessory") or hit.Parent:IsA("Tool") then return end
...
local function ContinueChasing(TargetedPlayer: Player)
	...
	if config.enemy_human.Health <= 0 then
		return false
	end

	npcInChase.Value = true
	npcPersistence.Value = 2
	...
	persistence.Value = 3
	config.sightFactor = config.enragedSightFactor
...
local function LostPlayer(TargetedPlayer: Player, overrideNetworkReset: boolean)
	...
	local persistence = inChase:FindFirstChild("Persistence")
	persistence.Value = 3
	config.sightFactor = config.wanderSightFactor
...
local function Chase(player: Player?)
	...
	local function coreTrack_UnableToPath()
		core_failures += 1
		if core_failures > 10 then return end
		--print("Rerouting!")
		isWandering = false
		isChasing = false
	...
	local function trackPlayer()
		if #config.standardPathfindSettings > 0 then 
			ai.SmartPathfind(config.enemy_char, player.Character, false, {Tracking = true, StandardPathfindingSettings = config.standardPathfindSettings, PredictiveMoveTo = 5, SMMD_RaycastParams = {range = 25, filterTable = {config.enemy_char, chasePart}}, Visualize = config.Visualize, Hooks = {UnableToPath = dummy_coreTrack_UnableToPath}}) -- start player chase.
		else
			ai.SmartPathfind(config.enemy_char, player.Character, false, {Tracking = true, SMMD_RaycastParams = {range = 25, filterTable = {config.enemy_char, chasePart}}, PredictiveMoveTo = 5, Visualize = config.Visualize, Hooks = {UnableToPath = dummy_coreTrack_UnableToPath}}) -- start player chase.
		end
	...
	while isChasing do
		...
		local nearest = GetNearestVisiblePlayer()
		if nearest and nearest ~= player then
			stopChasing()
			Chase(nearest)
		end
		...
		if std.math.LineOfSight(config.enemy_char, plr_char, {range = config.detectionRange * config.sightFactor + detectionFactor * config.penaltyFactor, SeeThroughTransparentParts = config.seeThroughTransparent, filterTable = {config.enemy_char, chasePart}}) then
			...
			spawn(function()
				...
				if #config.standardPathfindSettings > 0 then 
					result = ai.SmartPathfind(config.enemy_char, chasePart, true, {SkipToWaypoint = 2, StandardPathfindingSettings = config.standardPathfindSettings, SMMD_RaycastParams = {range = 25}, Visualize = config.Visualize, PredictiveMoveTo = 5}) -- start player chase.
				else
					result = ai.SmartPathfind(config.enemy_char, chasePart, true, {SkipToWaypoint = 2, SMMD_RaycastParams = {range = 25, Visualize = config.Visualize}, PredictiveMoveTo = 5}) -- start player chase.
				end
...
HOOKS_IN.PauseAI = function(optionalPauseTimer: number)
	...
	if optionalPauseTimer == nil then HOOKS_IN.ResumeAI() return end
	if optionalPauseTimer <= 0 then HOOKS_IN.ResumeAI() return end
